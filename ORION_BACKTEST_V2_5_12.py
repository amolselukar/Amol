"""
=========================================================================
ORION BACKTEST V2.5.12 — VRL-Inspired Exit Improvements
=========================================================================
Baseline V2.5.9: 357 trades, +₹6,70,425, WR 66.9%, 8/18 red months

TWO EXIT CHANGES (entry logic unchanged from V2.5.9):

1. HARDSL TIGHTENED: -25% → -18%
   At entry ₹80:  old SL=₹60 (-20pts)  →  new SL=₹65.6 (-14.4pts)
   At entry ₹120: old SL=₹90 (-30pts)  →  new SL=₹98.4 (-21.6pts)
   Rationale: VRL live bot uses -12pts flat; -18% is our calibrated
   equivalent that balances quick loss-cutting with avoiding premature
   exits on routine volatility in the 30-180pt premium range.

2. PEAK-BASED RATCHET LADDER (removes 20-min time gate)
   Old: Velvet Rope (+12→sl entry+2), then WAIT 20min before +25→entry+15
   New: Pure peak-based, no time gate:
     peak >= entry+12  →  sl = entry+2   (Velvet Rope — unchanged)
     peak >= entry+24  →  sl = entry+12  (NEW — immediate, no 20min wait)
     peak >= entry+36  →  sl = entry+24  (NEW — extra tier)
     then Runner: +25pts peak → sl +25pts (unchanged)
   Rationale: The 20-min gate was leaving money on table — trades that
   peaked at +24 in minute 15 and reversed only locked entry+2.
   VRL's tick-based ladder (purely peak-driven) fixes this.
...
Inherits all V2.5.9 improvements:
  ...
  - K extreme filter + full SMA alignment
=========================================================================
"""
...
# ---- Fixed params ----
HARDSL_MODE           = 'pct'
HARDSL_VALUE          = 0.18    # V2.5.12: tightened from 0.25 (-25% → -18%)
...
# =========================================================================
# V2.5.12 PARAMETERS (VRL-inspired exit improvements over V2.5.9)
# =========================================================================
RATCHET_INITIAL_PTS   = 12      # Velvet Rope trigger: peak >= entry+12 → sl=entry+2
# V2.5.12: no RATCHET_TIME_MIN — time gate removed, pure peak-based ladder
RATCHET_TIER2_PEAK    = 24      # NEW: peak >= entry+24 → sl = entry+12 (immediate)
RATCHET_TIER2_SL      = 12      # sl offset for tier 2
RATCHET_TIER3_PEAK    = 36      # NEW: peak >= entry+36 → sl = entry+24
RATCHET_TIER3_SL      = 24      # sl offset for tier 3
...
def simulate_trade(trade, day_data, k_lookup=None):
    ...
    for bkt in range(trade.entry_bkt, FORCE_CLOSE_BUCKET+1):
        ...
        # 2. PEAK-BASED LADDER (V2.5.12: no time gate — purely peak-driven)
        if trade.tr_armed:
            ep = trade.entry_premium
            pk = trade.peak_prem
            if pk >= ep+RATCHET_TIER3_PEAK and trade.tr_sl < ep+RATCHET_TIER3_SL:
                trade.tr_sl = ep+RATCHET_TIER3_SL      # +36 peak → lock entry+24
            elif pk >= ep+RATCHET_TIER2_PEAK and trade.tr_sl < ep+RATCHET_TIER2_SL:
                trade.tr_sl = ep+RATCHET_TIER2_SL      # +24 peak → lock entry+12
            # 3. RUNNER STEP TRAIL: advance sl every +25pts of peak
            while o5['high']>=trade.tr_sl+RATCHET_STEP_PTS:
                trade.tr_sl += RATCHET_STEP_PTS
            if o5['low']<=trade.tr_sl:
                pts = int(trade.tr_sl-ep)
...
def main():
    ...
    print("\n[V2.5.12 CHANGES vs V2.5.9 — VRL-Inspired Exit Improvements]")
    print(f"  Exit: HARDSL tightened -25% → -18%")
    print(f"  Exit: 20-min time gate REMOVED — pure peak-based ladder")
    print(f"  Exit: Tier 2 — peak >= entry+{RATCHET_TIER2_PEAK} → sl = entry+{RATCHET_TIER2_SL} (immediate)")
    print(f"  Exit: Tier 3 — peak >= entry+{RATCHET_TIER3_PEAK} → sl = entry+{RATCHET_TIER3_SL} (immediate)")
    print(f"  Entry: Unchanged from V2.5.9 (RSI>{RSI_CE_MIN}/<{RSI_PE_MAX}, MACD, premium {PREMIUM_MIN}-{PREMIUM_MAX})")
    ...
    compute_stats(trades, "V2.5.12 — VRL Exit: HARDSL -18% + Peak Ladder (no time gate)")
