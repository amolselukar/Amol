"""
=========================================================================
ORION BACKTEST V2.5.11 — Straddle Expansion Filter (IV Regime Gate)
=========================================================================
Baseline V2.5.9: 357 trades, +₹6,70,425, WR 66.9%, 8/18 red months

NEW IMPROVEMENT over V2.5.9:

4. STRADDLE EXPANSION FILTER — IV Regime Gate
   At 9:20 AM (bucket 1): record ATM straddle_ref = CE + PE
   At 9:45 AM (bucket 6): check straddle_chk = CE + PE
   If straddle_chk > straddle_ref × (1 + STRADDLE_HALT_PCT): skip all trades today

   RATIONALE:
   ATM straddle = market's implied uncertainty price. If it GROWS in the
   first 30 minutes, IV is rising — the market is getting more fearful.
   Option buyers pay elevated premium and face IV crush if the expected
   big move doesn't materialize intraday. The correct side on such days
   is selling, not buying.

   Straddle growth ≠ spot movement. When Nifty moves, CE+PE roughly
   cancel (delta hedge). ONLY IV expansion causes the total straddle
   to grow. 6% growth in 30 min = real fear, not noise.

   VALIDATION REPORT:
   This backtest also reports "Straddle-Halted Days" P&L — what we
   would have made/lost had we traded those days. This tells us:
   → Negative avoided P&L = filter is correctly blocking bad days ✓
   → Positive avoided P&L = filter is removing winners, reconsider ✗

=========================================================================
Inherits all V2.5.9: RSI gate, MACD 1h, premium gate, RS=25
=========================================================================
"""
...
# =========================================================================
# V2.5.9 PARAMETERS (inherits all V2.5.8 changes + RS=25 confirmed)
# =========================================================================
RATCHET_INITIAL_PTS   = 12      # V2.5.8: was 15
RATCHET_TIME_MIN      = 20      # V2.5.8: was 30
RATCHET_STEP_PTS      = 25      # V2.5.8 RS=25 confirmed best
CIRCUIT_BREAKER       = 3       # V2.5.8: was 4

# V2.5.8 entry filters (kept)
K_EXTREME_BARS        = 3
...
# V2.5.9 NEW entry filters
RSI_CE_MIN            = 53      # CE: RSI must be above this (bullish momentum)
RSI_PE_MAX            = 47      # PE: RSI must be below this (bearish momentum)
PREMIUM_MIN           = 30      # min entry premium — avoid deep OTM
PREMIUM_MAX           = 180     # max entry premium — avoid volatility spike entries
MACD_CONFIRM          = True    # require 1h MACD line > signal for CE, < for PE

# V2.5.11 — straddle expansion filter
STRADDLE_REF_BKT      = 1       # bucket 1  = 9:20 AM — first stable quote
STRADDLE_CHECK_BKT    = 6       # bucket 6  = 9:45 AM — 30 min after open
STRADDLE_HALT_PCT     = 0.06    # 6% straddle expansion → skip all buying today
...
class Trade:
    ...
    peak_prem: float = 0.0; closed: bool = False
...
def get_straddle_pct(day_data):
    """
    Returns (straddle_ref, straddle_chk, pct_change) comparing ATM straddle
    at STRADDLE_REF_BKT vs STRADDLE_CHECK_BKT.
    Returns (None, None, None) if data is unavailable.
    """
    nifty_5m = sorted(day_data['nifty_5m'], key=lambda b: b['bucket'])
    if not nifty_5m:
        return None, None, None
    spot = nifty_5m[0]['open']
    atm_target = int(round(spot / ATM_STEP)) * ATM_STEP
    opt = day_data['opt_5m']
    # Find strike with both CE and PE data
    avail_both = {k[0] for k in opt if k[1]=='CE'} & {k[0] for k in opt if k[1]=='PE'}
    if not avail_both:
        return None, None, None
    strike = min(avail_both, key=lambda s: abs(s - atm_target))
    ce_b = {b['bucket']: b for b in opt.get((strike, 'CE'), [])}
    pe_b = {b['bucket']: b for b in opt.get((strike, 'PE'), [])}
    ce_ref = ce_b.get(STRADDLE_REF_BKT, {}).get('open')
    pe_ref = pe_b.get(STRADDLE_REF_BKT, {}).get('open')
    ce_chk = ce_b.get(STRADDLE_CHECK_BKT, {}).get('close')
    pe_chk = pe_b.get(STRADDLE_CHECK_BKT, {}).get('close')
    if any(v is None or v <= 0 for v in [ce_ref, pe_ref, ce_chk, pe_chk]):
        return None, None, None
    straddle_ref = ce_ref + pe_ref
    straddle_chk = ce_chk + pe_chk
    return straddle_ref, straddle_chk, (straddle_chk - straddle_ref) / straddle_ref

def k_was_extreme(side, k_lookup, current_bkt, bars_back=K_EXTREME_BARS):
    for i in range(bars_back+1):
        k_val = k_lookup.get(current_bkt - i*3)
...
def run_day(day_date, day_data, df1h_prior, df1h_by_date, df15_by_date, df15_sorted):
    ...
    for bkt in range(len(nifty_5m)):
        ...
        macd_line=cand1h.get('MACD_line'); macd_sig=cand1h.get('MACD_sig')
        ...
        if len(df15a)>0 and not pd.isna(df15a['K'].iloc[0]):
            ...
            if Kp is not None:
                ...
                # V2.5.8: SMA full alignment
                ce_sma_ok = (cand1h['close']>s20) and (s20>s50)
                pe_sma_ok = (cand1h['close']<s20) and (s20<s50)

                sig_ce = ce_sma_ok and (Kn>=STOCHRSI_CE_LO) and (Kn>Kp)
                sig_pe = pe_sma_ok and (Kn<=STOCHRSI_PE_HI) and (Kn<Kp) and (Kn>=V2_K_FLOOR_PE)

                # V2.5.8: K extreme filter
                ...
                # =====================================================
                # V2.5.9 ENTRY FIX 1: RSI directional gate
                # CE: RSI must be genuinely bullish (>53), not just "not chop"
                # PE: RSI must be genuinely bearish (<47)
                # =====================================================
                if sig_ce and rsi_val is not None and rsi_val <= RSI_CE_MIN: sig_ce = False
                if sig_pe and rsi_val is not None and rsi_val >= RSI_PE_MAX: sig_pe = False

                # =====================================================
                # V2.5.9 ENTRY FIX 2: MACD 1h alignment
                # CE: MACD line must be above signal (bullish momentum)
                # PE: MACD line must be below signal (bearish momentum)
                # =====================================================
                if MACD_CONFIRM and macd_line is not None and macd_sig is not None:
        ...
        if chosen=='V3':
            ...
            # =====================================================
            # V2.5.9 ENTRY FIX 3: Premium gate
            # Avoid deep OTM (< 30) and volatility spike entries (> 180)
            # =====================================================
            if ep is None or not (PREMIUM_MIN <= ep <= PREMIUM_MAX): continue
...
def run_all(daily, df5_all, df15_all, df1h_all):
    ...
    all_trades=[]
    halted_trades=[]   # trades that WOULD have happened on straddle-halted days
    halted_days=[]     # (date, straddle_ref, straddle_chk, pct) for halted days
    for di,d in enumerate(dates):
        ...
        # V2.5.11: straddle expansion check
        s_ref, s_chk, s_pct = get_straddle_pct(dd)
        if s_pct is not None and s_pct >= STRADDLE_HALT_PCT:
            # Halted — still simulate to record what we avoided
            phantom = run_day(d,dd,df1h_prior,df1h_by_date,df15_by_date,df15s)
            halted_trades.extend(phantom)
            halted_days.append((d, s_ref, s_chk, s_pct))
            continue
        trades=run_day(d,dd,df1h_prior,df1h_by_date,df15_by_date,df15s)
        all_trades.extend(trades)
    return all_trades, halted_trades, halted_days

# -------------------- Stats --------------------
def compute_stats(trades, label=""):
    n=len(trades)
    if n==0:
        print(f"\n[WARNING] Zero trades generated — filters may be too tight."); return
...
def main():
    ...
    print("\n[V2.5.11 CHANGES vs V2.5.9]")
    print(f"  Entry: Straddle expansion halt — skip day if straddle grows >{STRADDLE_HALT_PCT*100:.0f}%")
    print(f"         Reference: bucket {STRADDLE_REF_BKT} (9:20AM)  Check: bucket {STRADDLE_CHECK_BKT} (9:45AM)")
    print(f"  Inherits: RSI>{RSI_CE_MIN}/<{RSI_PE_MAX}, MACD 1h, premium {PREMIUM_MIN}-{PREMIUM_MAX}, RS=25")
    print("\n[BASELINE PROGRESSION]")
    print("  V2.5.7:       912 trades | ₹+5,00,048 | WR 55.4% | RedMonths 10/18 | MaxDD -1885")
    print("  V2.5.8 RS=25: 486 trades | ₹+5,59,815 | WR 63.4% | RedMonths  6/18 | MaxDD -1384")
    print("  V2.5.9:       357 trades | ₹+6,70,425 | WR 66.9% | RedMonths  8/18 | MaxDD -1485")
    print("\n[Running V2.5.11...]")
    trades, halted_trades, halted_days = run_all(daily,df5,df15,df1h)
    compute_stats(trades, "V2.5.11 — Straddle Expansion Filter")

    # Straddle filter audit report
    print(f"\n{'='*70}")
    print(f"  STRADDLE FILTER AUDIT")
    print(f"{'='*70}")
    print(f"  Days halted by straddle filter : {len(halted_days)}")
    if halted_days:
        halted_pts = sum(t.pnl_nifty_pts() for t in halted_trades)
        halted_rs  = sum(t.pnl_prem_rs()   for t in halted_trades)
        halted_wins = sum(1 for t in halted_trades if t.pnl_prem_per_lot()>0)
        halted_n    = len(halted_trades)
        verdict = "GOOD — filter correctly blocked losses" if halted_pts < 0 else "BAD  — filter removed profitable trades"
        print(f"  Trades on halted days (phantom): {halted_n}")
        print(f"  Phantom P&L (pts)  : {halted_pts:+.0f}  →  {verdict}")
        print(f"  Phantom P&L (Rs)   : ₹{halted_rs:+,.0f}")
        if halted_n > 0:
            print(f"  Phantom Win Rate   : {halted_wins/halted_n*100:.1f}%")
        print(f"\n  Top 10 halted days (by straddle expansion):")
        top = sorted(halted_days, key=lambda x: x[3], reverse=True)[:10]
        for date, ref, chk, pct in top:
            day_trades = [t for t in halted_trades if t.day==date]
            day_pts = sum(t.pnl_nifty_pts() for t in day_trades)
            flag = "🔴" if day_pts<0 else "🟢"
            print(f"    {date}  straddle {ref:.0f}→{chk:.0f} (+{pct*100:.1f}%)  {flag} avoided {day_pts:+.0f}pts")
    print(f"{'='*70}")
