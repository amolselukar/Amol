"""
=========================================================================
ORION BACKTEST V2.5.10 — Macro Trend Filter (50-Day SMA Gate)
=========================================================================
Baseline V2.5.9: 357 trades, +₹6,70,425, WR 66.9%, 8/18 red months

ONE NEW IMPROVEMENT over V2.5.9:

4. MACRO TREND FILTER — 50-day daily SMA gate
   CE: skip entry if Nifty daily close < SMA50_daily (macro bearish regime)
   PE: skip entry if Nifty daily close > SMA50_daily (macro bullish regime)

   RATIONALE: V2.5.9 red month autopsy showed a clear pattern:
   - Oct 2024: Nifty -8% sustained fall → CE bounces trapped in bear market
   - Nov 2025: Strong global risk-off → CE entries on intraday rallies failed
   - Jan-Mar 2026: 3 consecutive reds = prolonged bear market CE failure
   These 5 months share one trait: Nifty was BELOW its 50-day SMA.
   Our MACD/RSI/SMA filters operate on 1h timeframe and generate valid
   intraday signals even in macro downtrends. The 50d SMA is the macro
   guardrail that prevents buying calls during bear market bounces.

   WHO EARNED THOSE MONTHS: Directional PE trend-followers (overnight
   positions, not intraday options buyers). Our oscillator-timed options
   entries cannot compete with sustained unidirectional moves — the right
   response is to stop fighting the macro trend, not improve entries.

=========================================================================
Inherits all V2.5.9 improvements:
  - RSI directional gate (CE: RSI>53, PE: RSI<47)
  - MACD 1h alignment confirmation
  - Premium entry gate (30 ≤ premium ≤ 180)
  - RS=25 ratchet step (confirmed from grid)
=========================================================================
"""
...
# V2.5.10 NEW entry filter
MACRO_SMA_PERIOD      = 50      # days for macro trend SMA (50-day daily close SMA)
...
def build_streams(daily):
    ...
    # Build daily OHLCV from 1h bars for macro trend filter
    df_daily = df1h.groupby('date', sort=True).agg(
        open=('open', 'first'), high=('high', 'max'),
        low=('low', 'min'), close=('close', 'last')
    ).reset_index()
    df_daily['SMA50_daily'] = df_daily['close'].rolling(MACRO_SMA_PERIOD).mean()
    return df5, df15, df1h, df_daily
...
# -------------------- Day Loop --------------------
def run_day(day_date, day_data, df1h_prior, df1h_by_date, df15_by_date, df15_sorted, df_daily_prior=None):
    ...
    # V2.5.10: macro trend state — block CE in macro downtrend, PE in macro uptrend
    macro_bull = macro_bear = None
    if df_daily_prior is not None and len(df_daily_prior) >= MACRO_SMA_PERIOD:
        last_d = df_daily_prior.iloc[-1]
        sma50_d = last_d['SMA50_daily']
        close_d = last_d['close']
        if not pd.isna(sma50_d):
            macro_bull = bool(close_d > sma50_d)
            macro_bear = bool(close_d < sma50_d)
    ...
    for bkt in range(len(nifty_5m)):
        ...
        if n15:
            for role,lvl in [('G',levels['G']),('R',levels['R'])]:
                ...
                if sig:
                    ...
                    # V2.5.10: macro filter applies to V3 entries too
                    if sig_dir_v3=='CE' and macro_bear: continue
                    if sig_dir_v3=='PE' and macro_bull: continue
                    sig_v3=(sig_dir_v3, lvl, sig['grade']); break
        ...
        if len(df15a)>0 and not pd.isna(df15a['K'].iloc[0]):
            ...
            if Kp is not None:
                ...
                # =====================================================
                # V2.5.10 ENTRY FIX 4: Macro 50-day SMA trend gate
                # CE blocked if Nifty daily close < 50-day SMA (bear market)
                # PE blocked if Nifty daily close > 50-day SMA (bull market)
                # =====================================================
                if sig_ce and macro_bear: sig_ce = False
                if sig_pe and macro_bull: sig_pe = False
...
# -------------------- Pipeline --------------------
def run_all(daily, df5_all, df15_all, df1h_all, df_daily_all):
    ...
    for di,d in enumerate(dates):
        ...
        df_daily_prior=df_daily_all[df_daily_all['date']<d].reset_index(drop=True)
        trades=run_day(d,dd,df1h_prior,df1h_by_date,df15_by_date,df15s,df_daily_prior)
...
def main():
    ...
    df5,df15,df1h,df_daily=build_streams(daily)
    print(f"[DATA] {len(daily)} sessions loaded.")
    print("\n[V2.5.10 CHANGES vs V2.5.9]")
    print(f"  Entry: Macro 50-day SMA gate — CE blocked if close < SMA50_daily")
    print(f"                                  PE blocked if close > SMA50_daily")
    print(f"  Inherits: RSI gate RSI>{RSI_CE_MIN}/<{RSI_PE_MAX}, MACD 1h, premium {PREMIUM_MIN}-{PREMIUM_MAX}, RS=25")
    ...
    print("\n[Running V2.5.10...]")
    trades=run_all(daily,df5,df15,df1h,df_daily)
    compute_stats(trades, "V2.5.10 — Macro 50-Day SMA Trend Gate")
