...
def check_vwap_signal(df15m_nifty, expiry_lookup, spot, df15m_fut=None):
    ...
    GATE 1 (PRIMARY): Nifty FUTURES 15m close crosses futures VWAP, body >= VWAP_BODY_MIN_PCT
    ...
    NOTE: Backtest only validated Gate 1. Gates 2+3 under evaluation.
    """
    ...
    linfo(f"[VWAP] TRIPLE confirmed — FUT {fc:.0f}{'>' if fut_bullish else '<'}{fut_vwap:.0f} "
          f"body={f_body_pct:.0%} SPOT {sc:.0f} OPT {opt_ltp:.1f} side={side}")
    return {
