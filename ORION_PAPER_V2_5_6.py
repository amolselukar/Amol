...
# V2.5.5: CHOP FILTER (RSI [47,53] indecision band block)
CHOP_FILTER_ENABLED   = True
CHOP_RSI_LO           = 47        # block entries when 1h RSI in [LO, HI]
CHOP_RSI_HI           = 53
...
# ENTRY DECISION
# =========================================================================
def chop_filter_blocks(df1h) -> bool:
    """V2.5.5 chop filter: block all entries when last closed 1h RSI in [LO, HI]."""
    if not CHOP_FILTER_ENABLED: return False
    if df1h is None or len(df1h) < 2 or 'RSI' not in df1h.columns: return False
    rsi_val = df1h['RSI'].iloc[-2]
    if pd.isna(rsi_val): return False
    rsi_val = float(rsi_val)
    return CHOP_RSI_LO <= rsi_val <= CHOP_RSI_HI

def check_v2_signal(df1h, df15m):
    """V2 entry: 1h SMA regime + 15m StochRSI K signal. V2.5.1 adds PE_floor=25.
    Uses iloc[-2] closed bars."""
