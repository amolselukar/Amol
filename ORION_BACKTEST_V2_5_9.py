"""
=========================================================================
ORION BACKTEST V2.5.9 — Three Knowledge-Driven Entry Improvements
=========================================================================
Baseline V2.5.8 (RS=25): 486 trades, +₹5,59,815, WR 63.4%, 6/18 red months

THREE NEW IMPROVEMENTS over V2.5.8:

1. MACD 1H ALIGNMENT (signal already computed, never used until now)
   CE: MACD_line > MACD_sig on 1h (momentum genuinely bullish)
   PE: MACD_line < MACD_sig on 1h (momentum genuinely bearish)
   → Filters entries where price is above SMA20 but momentum is fading
   → MACD crossover is a proven secondary confirmation for StochRSI signals

2. RSI DIRECTIONAL GATE (replaces simple chop band)
   CE: RSI > 53 (genuinely bullish momentum, not just "not chop")
   PE: RSI < 47 (genuinely bearish momentum)
   → Old filter: block if 47-53. New: require directional bias.
   → Entries where RSI is 54 but falling are now also blocked for CE

3. OPTION PREMIUM ENTRY GATE
   Valid entry: 30 ≤ premium ≤ 180
   → Below 30: deep OTM, won't move enough to cover transaction costs
   → Above 180: paying too much premium, time decay works against us fast,
     and it signals we may be entering during a volatility spike (bad timing)
=========================================================================
"""
...
def main():
    ...
    print("\n[V2.5.9 CHANGES vs V2.5.8]")
    print(f"  Exit:  RS=25 (confirmed from grid — was 15 in V2.5.8)")
    print(f"  Entry: RSI directional gate — CE: RSI>{RSI_CE_MIN}, PE: RSI<{RSI_PE_MAX}")
    print(f"  Entry: MACD 1h alignment — CE: MACD_line>MACD_sig, PE: MACD_line<MACD_sig")
    print(f"  Entry: Premium gate — {PREMIUM_MIN} ≤ premium ≤ {PREMIUM_MAX}")
    ...
    compute_stats(trades, "V2.5.9 — MACD Confirm + RSI Direction + Premium Gate")
